...
public class PythonStarlarkApiTest extends BuildViewTestCase {
  ...
  public void pythonProviderWithFields() throws Exception {
    ...
    Info provider = getPythonSkylarkProvider(helloTarget);

    assertThat(provider.hasKey(PyCommon.TRANSITIVE_PYTHON_SRCS)).isTrue();
    assertThat(provider.hasKey(PyCommon.IS_USING_SHARED_LIBRARY)).isTrue();
    assertThat(provider.hasKey(PyCommon.IMPORTS)).isTrue();
    assertThat(provider.hasKey("srcs")).isFalse();
  }
  ...
  public void simpleFieldsValues() throws Exception {
    ...
    SkylarkNestedSet sources = provider.getValue(PyCommon.TRANSITIVE_PYTHON_SRCS, SkylarkNestedSet.class);
  ...
  public void transitiveFieldsValues() throws Exception {
    ...
    SkylarkNestedSet sources = provider.getValue(PyCommon.TRANSITIVE_PYTHON_SRCS, SkylarkNestedSet.class);
    assertThat(prettyArtifactNames(sources.getSet(Artifact.class))).containsExactly("py/hello.py", "py/sayHello.py");
  ...
  private void simpleSources() throws IOException {
    scratch.file("py/hello.py",
        "import os",
        "def Hello():",
        "    print(\"Hello, World!\")",
        "    print(\"Hello, \" + os.getcwd() + \"!\")");
    scratch.file("py/sayHello.py",
        "from py import hello",
        "hello.Hello()");
    scratch.file("py/BUILD",
        "py_binary(name=\"sayHello\", srcs=[\"sayHello.py\"], deps=[\":hello\"])",
        "py_library(name=\"hello\", srcs=[\"hello.py\"], imports= [\".\"])");
  }

  private Info getPythonSkylarkProvider(ConfiguredTarget target) {
    Object object = target.get(PyCommon.PYTHON_SKYLARK_PROVIDER_NAME);
    assertThat(object).isInstanceOf(Info.class);
    Info provider = (Info) object;
